...
public class UsuarioDAO extends ConexionBD implements InterfaceCRUD {
    ...
    public UsuarioVO validarAcceso(String nombre, String contrasena) {
        ...
        try {
            mensajero = puente.executeQuery("SELECT * FROM USUARIOS WHERE NOMBREUSU = '"+nombre+"' AND CLAVEUSU = '"+contrasena+"' AND ESTADOUSU = 1");
            if(mensajero.next()){
                idUsu = mensajero.getString(1);
                nombreUsu = mensajero.getString(2);
                claveUsu = mensajero.getString(3);
    ...
    public boolean insertarRegistro() {
        try {
            puente.executeUpdate("INSERT INTO USUARIOS (IDUSU, NOMBREUSU, CLAVEUSU, ESTADOUSU) VALUES ('"+idUsu+"', '"+nombreUsu+"', '"+claveUsu+"', '"+estadoUsu+"');");
            operacion = true;
